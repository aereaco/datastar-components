...
//    - Props: Attributes prefixed with `data-component-prop-` are observed and made available
//      as reactive signals via a `$props` object within the component's scope.
//
// Performance Features:
// - Conditional loading of components using `data-component-load-if` based on Datastar signals.
...
//    - NEW: Fallback UI rendering via `data-component-fallback` attribute.
...
class DatastarComponent extends HTMLElement {
    ...
    connectedCallback() {
        ...
        // Execute data-component-connected expression if present
        const connectedExpression = this.getAttribute('data-component-connected');
        if (connectedExpression && Datastar) {
            try {
                // Evaluate the Datastar expression in the component's scope
                Datastar.evaluate(connectedExpression, Datastar.scope(this));
            } catch (e) {
                console.error(`[datastar-components] Error evaluating data-component-connected expression for ${this.tagName} (Source: ${this._componentSrc}):`, e);
            }
        }
    }

    disconnectedCallback() {
        // Execute data-component-disconnected expression if present
        const disconnectedExpression = this.getAttribute('data-component-disconnected');
        if (disconnectedExpression && Datastar) {
            try {
                // Evaluate the Datastar expression in the component's scope
                Datastar.evaluate(disconnectedExpression, Datastar.scope(this));
            } catch (e) {
                console.error(`[datastar-components] Error evaluating data-component-disconnected expression for ${this.tagName} (Source: ${this._componentSrc}):`, e);
            }
        }
...
function parseComponentHTML(htmlString, tagName, componentSrc) {
    ...
    if (templateElement) {
        ...
        if (templateElement.content.querySelector('[data-component-form-associated]') || templateElement.hasAttribute('data-component-form-associated')) {
             formAssociated = true;
        }
        ...
        window.dispatchEvent(new CustomEvent(EVENT_COMPONENT_LOAD_ERROR, {
            ...
            detail: {
                ...
                type: 'parsing-error',
                originalError: new Error('No <template> tag found')
            }
        }));
        ...
        if (newScript.type === 'module' && newScript.hasAttribute('src')) {
            import(newScript.src)
                ...
                .catch(error => {
                    ...
                    const fallbackSrc = componentInstance.getAttribute('data-component-fallback'); // Renamed attribute
                    if (fallbackSrc) {
...
/**
 * Sets up reactivity for data-component-prop- attributes on a component instance.
...
function setupPropsReactivity(componentInstance, dsInstance) {
    ...
    Array.from(componentInstance.attributes).forEach(attr => {
        if (attr.name.startsWith('data-component-prop-')) { // Renamed attribute prefix
            const propName = attr.name.substring('data-component-prop-'.length);
    ...
    const observer = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            if (mutation.type === 'attributes' && mutation.attributeName.startsWith('data-component-prop-')) { // Renamed attribute prefix
                const propName = mutation.attributeName.substring('data-component-prop-'.length);
...
async function defineComponent(tagName, componentSrc, originalElement) {
    ...
    if (errorDuringFetchOrParse) {
        const fallbackSrc = originalElement.getAttribute('data-component-fallback'); // Renamed attribute
    ...
    customElements.define(tagName, class extends DatastarComponent {
        ...
        connectedCallback() {
            super.connectedCallback(); // Call HTMLElement's connectedCallback. Note: base DatastarComponent's connectedCallback no longer calls super.connectedCallback() itself.
            ...
            if (!this._dsContentAttached) {
                ...
                if (typeof this.contentReadyCallback === 'function') {
                    ...
                    // This allows contentReadyCallback to be defined as an exported function in a module script
                    Datastar.scope(this).contentReadyCallback();
                }

                this._dsContentAttached = true;

                // Set initial value for form-associated elements if `value` attribute is present
...
function scanAndDefineComponents(node) {
    ...
    // Also query inside the node for nested components
    ...
    componentsToDefine.forEach(el => {
        ...
        const loadIfExpression = el.getAttribute('data-component-load-if'); // Renamed attribute
        ...
        if (loadIfExpression && Datastar) {
            try {
                ...
                if (!shouldLoad) {
                    console.log(`[datastar-components] Component ${tagName} (Source: ${componentSrc}) not loaded due to data-component-load-if condition.`);
                ...
                console.error(`[datastar-components] Error evaluating data-component-load-if for ${tagName} (Source: ${componentSrc}):`, e);
...
export function initDatastarComponents(datastarInstance) {
    ...
    // Warn if Constructable Stylesheets API is not fully supported
    ...
    // Initial scan of the document body for components
    scanAndDefineComponents(document.body);

    // Observe future DOM changes for dynamically added components
    const domMutationObserver = new MutationObserver(mutationsList => {
        for (const mutation of mutationsList) {
            ...
            } else if (mutation.type === 'attributes' && mutation.attributeName === 'data-component-load-if') { // Watch for changes to data-component-load-if
                // Re-evaluate the data-component-load-if condition if the attribute changes
                scanAndDefineComponents(mutation.target);
            }
        }
    });

    // Observe the entire document body for childList mutations (additions/removals of elements)
    // and attribute changes to data-component-load-if on any element within the subtree.
    domMutationObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-component-load-if'] });
}
